...
def force_dirty(self, context):
    ...
    for pmod in pmods.values():
        pmd = pmod.get_pmod()
        if pmd != None and pmd.name == 'Mirror Rotation':
            pmd.set_dirty(True)

class ERNST_PG_PModifier_MirrorRotation(PropertyGroup):
    name: bpy.props.StringProperty(default = 'Mirror Rotation')
    icon: bpy.props.StringProperty(default = 'CON_ROTLIMIT')
    ...
    value: bpy.props.FloatProperty(name = 'value', min = math.pi*-1, max = math.pi, default = 0, update = force_dirty)

    def get_uniform_names(self, target_cp_name, id):
        side = 'P' if self.target_axis[0] == '+' else 'N'
        target_axis_elem = self.target_axis[1:].upper()
        u_mirror_Rotation_name = f'{target_cp_name}_mirror{id}{target_axis_elem}_{side}_rot{self.axis.upper()}'
        return u_mirror_Rotation_name
    ...
    def draw_gui(self, layout, obj, id, active):
        ...
        col = row.column(align=True)
        col.prop(self, 'value', text='Rotation')
