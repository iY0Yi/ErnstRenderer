...
def force_dirty(self, context):
    ...
    for pmod in pmods.values():
        pmd = pmod.get_pmod()
        if pmd != None and pmd.name == 'Elongate':
            pmd.set_dirty(True)

class ERNST_PG_PModifier_Elongate(PropertyGroup):
    name: bpy.props.StringProperty(default = 'Elongate')
    icon: bpy.props.StringProperty(default = 'STICKY_UVS_VERT')
    ...
    length: bpy.props.FloatProperty(name = 'Length', min = 0.01, max = 10.0, default = 1.0, update = force_dirty)

    def get_uniform_names(self, target_cp_name, id):
        u_name = f'{target_cp_name}_elongate{self.axis.upper()}_length'
        return u_name
    ...
    def update_uniforms(self, shader, target_cp_name, id):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_name] = (self.length, 0, 0, 0)
                self.set_dirty(False)
        else:
            uniform_float(shader, u_name, self.length)
    ...
    def get_shader_code(self, obj, target_cp_name, id, is_fixed):
        ...
        loader = sgd.module_lib.pmod['PMOD_ELONGATE']
        loader.used = True
        return '{0}({1}.{2}, {3});\n'.format(
            loader.fncname,
            target_cp_name,
