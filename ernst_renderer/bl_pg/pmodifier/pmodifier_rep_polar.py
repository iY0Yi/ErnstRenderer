...
def force_dirty(self, context):
    ...
    for pmod in pmods.values():
        pmd = pmod.get_pmod()
        if pmd != None and pmd.name == 'RepPolar':
            pmd.set_dirty(True)

class ERNST_PG_PModifier_RepPolar(PropertyGroup):
    name: bpy.props.StringProperty(default = 'RepPolar')
    ...
    repetation: bpy.props.IntProperty(name = 'Repetation', min = 1, max = 100, default = 1, update = force_dirty)

    def get_uniform_names(self, target_cp_name, id):
        u_name = f'{target_cp_name}_rep_polar{self.axis.upper()}'
        return u_name
    ...
    def draw_gui(self, layout, obj, id, active):
        ...
        col = row.column(align=True)
        col.prop(self, 'repetation', text='Repetation')
    ...
    def update_uniforms(self, shader, target_cp_name, id):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_name] = (self.repetation, 0, 0, 0)
                self.set_dirty(False)
        else:
            uniform_float(shader, u_name, self.repetation)
    ...
    def get_shader_code(self, obj, target_cp_name, id, is_fixed):
        ...
        loader = sgd.module_lib.pmod['PMOD_REP_POLAR']
        loader.used = True
        return '{0}({1}.{2}, {3});\n'.format(
            loader.fncname,
            target_cp_name,
