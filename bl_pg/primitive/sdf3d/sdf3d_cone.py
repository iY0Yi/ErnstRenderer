...
class ERNST_PG_SDF3D_Cone(PropertyGroup, ERNST_PG_SDF_Abstract):
    ...
    def update_uniforms(self, shader):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_names['radiuses_height']] = (obj.data.wData.rad_2*obj.scale[0], obj.data.wData.rad_1*obj.scale[0], obj.dimensions[2]*.5,0)
            ...
            uniform_float(shader, u_names['height'], obj.dimensions[2]*.5)
    ...
    def get_shader_code(self, rendables):
        ...
        if is_fixed(obj):
            ...
            height = er_f(obj.dimensions[2]*.5)
        else:
            if ubo.enabled:
        ...
        primitive = '{0}({1}, {2}{5}{6}, {3}{5}{6}, {4}{5}{6})'.format(
            loader.fncname,
            local_domain_name,
            height,
            radius_bottom,
