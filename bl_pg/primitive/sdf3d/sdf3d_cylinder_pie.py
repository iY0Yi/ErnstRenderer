...
class ERNST_PG_SDF3D_CylinderPie(PropertyGroup, ERNST_PG_SDF_Abstract):
    icon: bpy.props.StringProperty(default = 'MESH_CYLINDER')
    percentage: bpy.props.FloatProperty(name = 'percentage', min = 0.0, max = 100.0, default = 50., update = force_dirty)
    ...
    def get_uniform_names(self):
        ...
        u_names['percentage'] = f'{er_var(obj.name)}_percentage'
        u_names['radius_height_percentage'] = f'{er_var(obj.name)}_radius_height_percentage'
    ...
    def draw_gui(self, context, layout):
        ...
        col.prop(self, 'percentage', text='Percentage')
    ...
    def get_uniform_dec_code(self):
        ...
        if ubo.enabled:
            code_u_names  = ''
            ubo.add_vec4(u_names["radius_height_percentage"])
            self.add_ubo_shell()
            self.add_ubo_round()
            ...
            code_u_names += f'uniform float {u_names["percentage"]};\n'
    ...
    def update_uniforms(self, shader):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_names['radius_height_percentage']] = (obj.dimensions[0]*0.5, obj.dimensions[2]*0.5, math.pi*.9999*obj.ernst.shader_proxy.props.percentage*.01,0)
            ...
            uniform_float(shader, u_names['percentage'], math.pi*.9999*obj.ernst.shader_proxy.props.percentage*.01)
    ...
    def get_shader_code(self, rendables):
        ...
        if is_fixed(obj):
            ...
            percentage = er_f(math.pi*.9999*obj.ernst.shader_proxy.props.percentage*.01)
        else:
            if ubo.enabled:
                radius = '('+ubo.name(u_names['radius_height_percentage'])+'.x)'
                height = '('+ubo.name(u_names['radius_height_percentage'])+'.y)'
                percentage = '('+ubo.name(u_names['radius_height_percentage'])+'.z)'
            else:
                radius = u_names['radius']
                height = u_names['height']
                percentage = u_names['percentage']
        ...
        primitive = '{0}({1}, {2}{5}{6}, {3}{5}{6}, {4}){5}'.format(
            loader.fncname,
            local_domain_name,
            radius,
            height,
