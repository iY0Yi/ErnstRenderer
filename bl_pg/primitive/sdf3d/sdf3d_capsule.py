...
class ERNST_PG_SDF3D_Capsule(PropertyGroup, ERNST_PG_SDF_Abstract):
    icon: bpy.props.StringProperty(default = 'META_CAPSULE')
    ...
    def update_uniforms(self, shader):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_names['radius_height']] = (obj.dimensions[0]*0.5, obj.dimensions[2]*0.5 - obj.dimensions[0]*.5, 0,0)
                self.update_ubo_shell()
                self.update_ubo_boolean()
                self.set_dirty(False)
        else:
            uniform_float(shader, u_names['radius'], obj.dimensions[0]*0.5)
            uniform_float(shader, u_names['height'], obj.dimensions[2]*0.5 - obj.dimensions[0]*.5)
    ...
    def get_shader_code(self, rendables):
        ...
        if is_fixed(obj):
            ...
            height = er_f(obj.dimensions[2]*0.5 - obj.dimensions[0]*.5)
        else:
            if ubo.enabled:
