...
class ERNST_PG_SDF3D_TorusCapped(PropertyGroup, ERNST_PG_SDF_Abstract):
    ...
    def get_uniform_names(self):
        ...
        u_names['radiuses_percentage'] = f'{er_var(obj.name)}_radiuses_percentage'
    ...
    def get_uniform_dec_code(self):
        ...
        if ubo.enabled:
            code_u_names  = ''
            ubo.add_vec4(u_names["radiuses_percentage"])
            self.add_ubo_shell()
            self.add_ubo_round()
    ...
    def update_uniforms(self, shader):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_names['radiuses_percentage']] = (obj.dimensions[0]*.5-obj.dimensions[2]*.5, obj.dimensions[2]*.5, math.pi*.9999*obj.ernst.shader_proxy.props.percentage*.01,0)
    ...
    def get_shader_code(self, rendables):
        ...
        if is_fixed(obj):
            ...
            if ubo.enabled:
                radius_main = '('+ubo.name(u_names['radiuses_percentage'])+'.x)'
                radius_minor = '('+ubo.name(u_names['radiuses_percentage'])+'.y)'
                percentage = '('+ubo.name(u_names['radiuses_percentage'])+'.z)'
            else:
        ...
        primitive = '{0}({1}, vec2({2}, {3}){5}, {4})'.format(
            loader.fncname,
            local_domain_name,
            radius_main,
            radius_minor,
