...
class ERNST_PG_SDF_2D_Circle(PropertyGroup, ERNST_PG_SDF_2D_Abstract):
    ...
    def update_uniforms(self, shader):
        ...
        u_names = self.get_uniform_names()
        if self.axis == 'x':
            rad = obj.dimensions[1]*0.5
        if self.axis == 'y':
            rad = obj.dimensions[0]*0.5
        if self.axis == 'z':
            rad = obj.dimensions[0]*0.5
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_names['radius']] = (rad, 0,0,0)
                self.update_ubo_shell()
                self.update_ubo_boolean()
                self.set_dirty(False)
        else:
            uniform_float(shader, u_names['radius'], rad)
    ...
    def get_shader_code(self, rendables):
        ...
        if is_fixed(obj):
            ...
            if ubo.enabled:
                radius = '('+ubo.name(u_names['radius'])+'.x)'
            else:
