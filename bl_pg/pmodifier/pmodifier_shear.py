...
def force_dirty(self, context):
    ...
    for pmod in pmods.values():
        pmd = pmod.get_pmod()
        if pmd != None and pmd.name == 'Shear':
            pmd.set_dirty(True)

class ERNST_PG_PModifier_Shear(PropertyGroup):
    name: bpy.props.StringProperty(default = 'Shear')
    icon: bpy.props.StringProperty(default = 'OUTLINER_OB_LATTICE')
    ...
    strength: bpy.props.FloatProperty(name = 'Strength', min = -1, max = 1, default = 0, update = force_dirty)

    def get_uniform_names(self, target_cp_name, id):
        u_name = f'{target_cp_name}_shear{self.axis.upper()}'
        return u_name
    ...
    def get_shader_code(self, obj, target_cp_name, id, is_fixed):
        ...
        loader = sgd.module_lib.pmod['PMOD_SHEAR']
        loader.used = True
        return '{0}({1}.{2}, {3});\n'.format(
            loader.fncname,
            target_cp_name,
