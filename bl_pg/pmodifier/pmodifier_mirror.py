...
def force_dirty(self, context):
    ...
    for pmod in pmods.values():
        pmd = pmod.get_pmod()
        if pmd != None and pmd.name == 'Mirror':
            pmd.set_dirty(True)

class ERNST_PG_PModifier_Mirror(PropertyGroup):
    name: bpy.props.StringProperty(default = 'Mirror')
    icon: bpy.props.StringProperty(default = 'MOD_MIRROR')
    ...
    smoothness: bpy.props.FloatProperty(name = 'smoothness', min = 0, max = 5, default = 0, update = force_dirty)

    def get_uniform_names(self, target_cp_name, id):
        axis = self.axis.upper()
        u_name = f'{target_cp_name}_mirror{id}{axis}_smoothness'
        return u_name
    ...
    def draw_gui(self, layout, obj, id, active):
        ...
        col = row.column(align=True)
        col.prop(self, 'smoothness', text='Smooth')
    ...
    def update_uniforms(self, shader, target_cp_name, id):
        ...
        if ubo.enabled:
            ...
            if is_dirty[obj.name] or ubo.is_dirty:
                ubo.data[u_name] = (self.smoothness, 0, 0, 0)
                self.set_dirty(False)
        else:
            uniform_float(shader, u_name, self.smoothness)

    def get_shader_code(self, obj, target_cp_name, id, is_fixed):
        if self.visible==False:
            return ''
        loader = sgd.module_lib.pmod['PMOD_MIRROR']
        loader.used = True
        mirror_id = id
        u_name = self.get_uniform_names(target_cp_name, mirror_id)
        if is_fixed:
            v = er_f(self.smoothness)
        else:
            if ubo.enabled:
                v = f'ubo.{u_name}.x'
            else:
        ...
        code = ''
        code += f'vec3 {target_cp_name}_mir{mirror_id}_Pos = max(vec3(0), sign({target_cp_name}));\n'
        code += f'vec3 {target_cp_name}_mir{mirror_id}_Neg = max(vec3(0),-sign({target_cp_name}));\n'
        code += '{0}({1}.{2}, {3});\n'.format(
                loader.fncname,
                target_cp_name,
                self.axis,
