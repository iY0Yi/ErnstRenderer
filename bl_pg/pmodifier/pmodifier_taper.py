...
def force_dirty(self, context):
    ...
    for pmod in pmods.values():
        pmd = pmod.get_pmod()
        if pmd != None and pmd.name == 'Taper':
            pmd.set_dirty(True)

class ERNST_PG_PModifier_Taper(PropertyGroup):
    name: bpy.props.StringProperty(default = 'Taper')
    icon: bpy.props.StringProperty(default = 'MESH_CONE')
    ...
    def get_uniform_names(self, target_cp_name, id):
        u_name = f'{target_cp_name}_taper{self.axis.upper()}'
        return u_name
    ...
    def get_shader_code(self, obj, target_cp_name, id, is_fixed):
        ...
        loader = sgd.module_lib.pmod['PMOD_TAPER']
        loader.used = True
        return '{0}({1}.{2}, {3});\n'.format(
            loader.fncname,
            target_cp_name,
